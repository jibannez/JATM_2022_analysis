 ...
 if i > counter
  for j = 2:n      % Loop for columns
   if j > counter+1
    ...
    hold(gca, 'on');                    % Allow superimposed plots
    ...
    if i == 1      
     ylh = ylabel(regexprep(var(j), '_', '-'),'FontSize',labelsize+2);
    ...
    if j == n      
     xlh = xlabel(regexprep(var(i), '_', '-'),'FontSize',labelsize+2);
     set(xlh, 'Rotation',xlabelangle, 'VerticalAlignment','middle', 'HorizontalAlignment','right')
     ax = gca;
